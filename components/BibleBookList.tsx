...
export default function BibleBookList({books}: {books : Book[]}){
    ...
    return (
        <FlatList
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity onPress={()=>router.navigate(`/bible/${item.id}`)} className="p-1 pl-2 pr-2">
                            <Card className="p-4 mb-2 bg-background rounded-2xl">
                                <View className="flex-row items-center justify-between">
                                    ...
                                    <Text className="text-sm text-muted-foreground">{item.chapters} {t('bible.chapters')}</Text>
                                </View>
                                <View className="flex-row items-center justify-between mt-2">
                                    <Text className="text-sm text-muted-foreground">{t('bible.progress')}</Text>
                    ...
                    ListHeaderComponent={
                        <View className="flex-row items-center justify-end mb-2 pr-5">
                            <Text className="text-sm text-muted-foreground">{books.length} {t('bible.books')}</Text>
                        </View>
                    }
                    />
    )
