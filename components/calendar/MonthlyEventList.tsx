...
function isEventInMonth(
    ...
    if (viewMode === 'ethiopian') {
        ...
        const eventStartsBeforeMonthEnd =
            ...
            (start.year === monthEnd.year && start.month < monthEnd.month) ||
            (start.year === monthEnd.year &&
                start.month === monthEnd.month &&
                (start.day ?? 1) <= monthEnd.day);
        const eventEndsAfterMonthStart =
            end.year > monthStart.year ||
            (end.year === monthStart.year && end.month > monthStart.month) ||
            (end.year === monthStart.year &&
                end.month === monthStart.month &&
                (end.day ?? 1) >= monthStart.day);
...
export const MonthlyEventList: React.FC<MonthlyEventListProps> = ({
    ...
    function getEventName(ev: any) {
        if (ev.name) {
            ...
            if (typeof ev.name === 'object')
                return ev.name.am || Object.values(ev.name)[0] || ev.id;
        }
        return ev.id;
    }
